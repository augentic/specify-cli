...
use crate::workspace::git::git_output_ok;
...
pub(in crate::workspace) fn current_branch(project_path: &Path) -> Result<Option<String>, Error> {
...
pub(in crate::workspace) fn origin_head_branch(project_path: &Path) -> Option<String> {
    if let Some(full) =
