//! Typed-model drift domain logic for `specrun slice validate`
//! (RFC-29c §"Drift validation").
//!
//! Extracted from the CLI handler (REVIEW.md A15) so the drift, target,
//! source-orphan, cross-ref, claim-kind, and ID-grammar gates live in
//! the workflow layer next to the model types they read, leaving the
//! handler a thin `Ctx`-bound orchestrator. Every entry point takes
...
use crate::schema::evidence_yaml_paths;
