...
//!   (a platform hub's committed `.specify/topology.lock`, or the sole
...
/// ([`ProjectConfig::hub`]):
///
/// - **Hub** — one [`ProjectRef`] per entry in the committed
///   `.specify/topology.lock` (RFC-36), the projection of each member
///   project's `project.yaml` regenerated by `specrun workspace sync`.
///   `name`, `target`, `description`, `capabilities`, and `keywords`
///   come from the cache. An absent cache fails `topology-cache-missing`
///   directing the operator to run `workspace sync`.
/// - **Single regular project** — one synthesised [`ProjectRef`]:
///   `name` from `project.yaml.name`, `description` / `capabilities` /
///   `keywords` from `project.yaml`, and `target` formed by resolving
///   `project.yaml.adapter` through [`TargetAdapter::resolve`] and
///   joining the resolved adapter's canonical `name` + `version` into
///   `name@vN`. A regular project reads its own `project.yaml` live as
///   its single source of truth — no cache.
///
/// Both branches touch the filesystem: the hub branch reads the lock,
/// the regular branch resolves the target adapter under `project_dir`.
///
/// # Errors
///
/// - [`Error::Validation`] (`topology-cache-missing`) when a hub has no
...
/// [`ProjectRef`]. RFC-36: hub topology is derived from each member
/// project's `project.yaml`, not from `registry.yaml`.
fn hub_topology(project_dir: &Path) -> Result<Vec<ProjectRef>> {
    let path = Layout::new(project_dir).topology_lock_path();
    let lock = TopologyLock::load(&path)?.ok_or_else(|| {
        Error::validation_failed(
            "topology-cache-missing",
            "a hub has a committed .specify/topology.lock",
            "hub plan-time topology requires .specify/topology.lock; \
