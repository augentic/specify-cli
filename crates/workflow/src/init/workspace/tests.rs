...
#[test]
fn workspace_init_writes_canonical_on_disk_shape() {
...
#[test]
fn workspace_init_writes_default_wasm_pkg_config() {
    let tmp = tempdir().unwrap();
    let result = init(workspace_opts(tmp.path(), "platform-workspace"), fixed_now())
        .expect("workspace init ok");
...
#[test]
fn workspace_init_rejects_non_kebab_name() {
    let tmp = tempdir().unwrap();
    let err = init(workspace_opts(tmp.path(), "BadName"), fixed_now()).expect_err("non-kebab name");
    match err {
        specify_error::Error::Diag { code, detail } => {
