...
//!   `name:` field, and emit one [`specify_standards::Diagnostic`] per duplicated
...
//!   [`specify_standards::lint::Skill`] fact per well-formed SKILL.md
//!   (`crates/standards/src/lint/index/skill.rs::extract`); the
//!   `kind: unique` interpreter
//!   (`crates/standards/src/lint/eval/unique.rs::evaluate`) consumes
