...
//!   `crates/standards/src/framework/check/adapter.rs`) does not exist as a clean
...
//!   [`specify_standards::lint::AdapterManifest`] fact per well-formed
//!   manifest (`crates/standards/src/lint/index/adapter.rs::extract`,
//!   whose `brief_keys` field mirrors the `briefs:` map keys verbatim);
//!   the `kind: set-eq` interpreter
//!   (`crates/standards/src/lint/eval/set_eq.rs::evaluate`) consumes
