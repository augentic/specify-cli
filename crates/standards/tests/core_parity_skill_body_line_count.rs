...
//!   lines, and emit one [`specify_standards::Diagnostic`] per skill
...
//!   one [`specify_standards::lint::Skill`] fact per well-formed SKILL.md
//!   (`crates/standards/src/lint/index/skill.rs::extract`), whose
//!   `body_line_count` field is the same count of non-frontmatter
//!   body lines the imperative row used; the `kind: cardinality`
//!   interpreter
//!   (`crates/standards/src/lint/eval/cardinality.rs::evaluate`)
