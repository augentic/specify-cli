...
fn check_description_grammar(ctx: &Context) -> Result<Vec<Diagnostic>, ToolingError> {
    ...
    for entry in load_skill_entries(ctx)? {
        ...
        findings.push(finding(
            RULE_DESCRIPTION_GRAMMAR,
            format!(
                "Skill description must start with an imperative verb: {} — '{first_alpha}' not in allow-list (add to IMPERATIVE_VERBS in crates/standards/src/framework/check/skill_frontmatter.rs if it is genuinely imperative)", 
                entry.rel
            ),
            &entry.path,
        ));
