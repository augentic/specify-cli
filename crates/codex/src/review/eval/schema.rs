...
//!
//! `http(s)://` references are rejected so `specrun review` runs
...
use specify_schema::{
    ...
    REVIEW_RESULT_JSON_SCHEMA, WORKSPACE_MODEL_JSON_SCHEMA, compile_schema,
};
...
static REGISTERED_SCHEMAS: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    HashMap::from([
        ("codex-rule", CODEX_RULE_JSON_SCHEMA),
        ("resolved-codex", RESOLVED_CODEX_JSON_SCHEMA),
        ("review-finding", REVIEW_FINDING_JSON_SCHEMA),
        ("review-result", REVIEW_RESULT_JSON_SCHEMA),
...
fn compile_schema_for_hint(
    ...
    if raw.starts_with("http://") || raw.starts_with("https://") {
        return Err(HintError::SchemaResolve {
            ...
            reason: "external http(s) references are not allowed; specrun review must run offline"
                .to_owned(),
        });
    }
