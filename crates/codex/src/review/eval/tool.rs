...
//! lives in `specrun review` (S9); this module only consumes the
...
//! - Successful runs whose stdout is the RFC-28 `ReviewResult`
//!   envelope OR a single `ReviewFinding` body fold the tool's
...
///
/// `specrun review` (S9) supplies a `wasmtime`-backed implementation;
...
fn parse_tool_findings(output: &ToolOutput) -> Vec<ReviewFinding> {
    if output.stdout.is_empty() {
        return Vec::new();
    }
    if let Ok(envelope) = serde_json::from_slice::<ReviewResult>(&output.stdout) {
        return envelope.findings;
    }
    if let Ok(single) = serde_json::from_slice::<ReviewFinding>(&output.stdout) {
        return vec![single];
    }
    Vec::new()
}

fn build_undeclared(rule: &ResolvedRule, hint: &DeterministicHint, id_num: u64) -> ReviewFinding {
