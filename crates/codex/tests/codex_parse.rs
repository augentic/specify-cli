...
fn parses_every_universal_codex_rule() {
    ...
    let rules_dir = adapters.join("shared").join("codex").join("universal");
    ...
    for path in files {
        let rule = parse_codex_rule_file(&path)
...
fn parses_every_documentation_source_codex_rule() {
    ...
    let rules_dir = adapters.join("sources").join("documentation").join("codex");
    let files = collect_rule_files(&rules_dir);
    if files.is_empty() {
        eprintln!("skip: no rules under {}", rules_dir.display());
        return;
