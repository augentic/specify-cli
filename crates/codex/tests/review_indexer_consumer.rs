...
//!
//! Drives `review::index::build` against the checked-in
//! `fixtures/review/minimal/` tree, augmenting it at runtime with the
...
fn fixture_src() -> PathBuf {
    crate_root().join("tests/fixtures/review").join(FIXTURE_NAME)
}

fn golden_path() -> PathBuf {
    crate_root().join("tests/fixtures/review").join(format!("{FIXTURE_NAME}_workspace_model.json"))
}
