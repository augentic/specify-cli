...
/// Default package client backed by `wasm-pkg-client`.
...
async fn fetch(request: &PackageRequest, dest_hint: &Path) -> Result<FetchedPackage, ToolError> {
...
async fn config_for(package: &PackageRef) -> Result<Config, ToolError> {
    ...
    if let Some(path) = std::env::var_os("WKG_CONFIG") {
        let override_config = Config::from_file(&path).await.map_err(|err| ToolError::Package {
            source_value: package.to_string(),
