//! Integration tests for the RFC-25 plugin loader
//! (`specify_domain::plugin`).
//!
//! Covers:
...
use specify_domain::plugin::{Axis, Plugin, PluginLocation, cache_dir};
...
fn resolves_source_adapter_from_local_directory() {
    ...
    let resolved = Plugin::resolve(Axis::Source, "code-typescript", &project)
    ...
    assert!(matches!(resolved.location, PluginLocation::Local(_)));
...
fn resolves_target_adapter_from_local_directory() {
    let (_tmp, project) = local_project();
    let resolved = Plugin::resolve(Axis::Target, "omnia", &project)
...
fn axis_disambiguates_colliding_names() {
    ...
    let (_tmp, project) = local_project();
    let source = Plugin::resolve(Axis::Source, "foo", &project).expect("source/foo resolves");
    let target = Plugin::resolve(Axis::Target, "foo", &project).expect("target/foo resolves");
...
",
    ...
    let resolved =
        Plugin::resolve(Axis::Source, "code-typescript", &project).expect("resolve from cache");
    assert_eq!(resolved.manifest.version, 7, "cache wins over local");
    assert!(matches!(resolved.location, PluginLocation::Cached(_)));
}

#[test]
fn missing_plugin_reports_plugin_not_found() {
    let (_tmp, project) = local_project();
    let err = Plugin::resolve(Axis::Source, "nonexistent", &project)
...
",
    ...
    let err = Plugin::resolve(Axis::Source, "wrong-ops", &project)
        .expect_err("source-axis adapter with wrong operations must fail");
    let detail = err.to_string();
    assert!(
        detail.contains("plugin-schema-violation") || detail.contains("plugin-manifest-malformed"),
...
",
    ...
    let err = Plugin::resolve(Axis::Source, "mislabeled", &project)
        .expect_err("axis literal must match the requested axis");
    let detail = err.to_string();
    assert!(
        detail.contains("plugin-schema-violation") || detail.contains("plugin-axis-mismatch"),
