//! `NestJS` detector integration tests.
//!
//! Regenerate goldens:
//! `REGENERATE_GOLDENS=1 cargo nextest run -p specify-domain --test detectors_nestjs`

use std::collections::HashSet;
use std::fs;
use std::path::PathBuf;

use specify_domain::survey::detectors::NestJsDetector;
use specify_domain::survey::{Detector, DetectorInput, SurfacesDocument, validate_surfaces};

fn fixtures_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/detectors/nestjs")
}
...
#[test]
fn detects_nestjs_routes_golden() {
...
fn skips_when_no_package_json() {
    ...
    let output = NestJsDetector.detect(&input).unwrap();
    assert!(output.surfaces.is_empty());
}

#[test]
fn skips_when_nestjs_not_dependency() {
    let dir = tempfile::TempDir::new().unwrap();
