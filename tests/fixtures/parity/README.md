...
The `case-10-design-refs` empty `expected-validation.txt` is a deliberate Python-era quirk (see the parity-quirk comment in `crates/workflow/src/merge/validate.rs`); a correct orphan-reference check lives in `specify_validate` (rule `cross.design-references-valid`).
