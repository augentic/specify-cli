...
The release workflow publishes those package requests through `wkg`; `specrun tool fetch` resolves them through the embedded first-party namespace default and Augentic registry metadata. Maintainers may use `wkg` for manual inspection, but operators only need the `specrun` runtime binary.
