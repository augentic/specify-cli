...
Wave 0.2 (`cli/W0.2`) renamed `Adapter*` → `Target*` for the output-role domain types (`Target`, the `Slice.target` field, the `slice-create-target-missing` error discriminant, plus every fixture, JSON envelope, and call site). Specify 2.0 then settled the regular/workspace init guard on its documented spelling `init-requires-adapter-or-workspace` (the Wave 0.2-era `init-requires-target-or-workspace` is gone). A later change dropped the per-slice `plan.yaml` `target` field entirely — a slice binds only a `project` and the target adapter is resolved on demand — retiring the `plan.entry-needs-project-or-target` discriminant in favour of `plan-reconcile-project-binding-required`.

Wave 0.3 (`cli/W0.3`) moved the shared manifest *shape* into a new axis-aware loader. The F9 collapse then retired the legacy axis-agnostic `crate::adapter` module (`Adapter` / `Pipeline` / `PipelineEntry` / `PipelineView` / `Phase` / `AdapterSource` / `ResolvedAdapter` / the legacy `adapter.schema.json`) and renamed the axis-aware loader back into `crates/workflow/src/adapter/` under the cleaner `SourceAdapter` / `TargetAdapter` / `Axis` / `ResolvedAdapter` / `AdapterLocation` type names.
...
`CacheMeta` was rehomed inside [`crates/workflow/src/init/cache.rs`](../../crates/workflow/src/init/cache.rs). The `Phase` enum was replaced by `Operation { Shape, Build, Merge }` on the slice-metadata wire (`phase: shape | build | merge`).
