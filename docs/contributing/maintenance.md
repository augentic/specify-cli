...
The mechanical surface is `cargo make standards-check` (predicates in [`xtask/src/standards.rs`](../../xtask/src/standards.rs), per-file baselines in [`scripts/standards-allowlist.toml`](../../scripts/standards-allowlist.toml)). Predicate semantics are documented in [AGENTS.md §"Mechanical enforcement"](../../AGENTS.md#mechanical-enforcement).
...
1. Run `cargo make standards-check` from the repo root. The footer prints per-predicate totals across the whole tree.
...
3. Cross-check by running `cargo make standards-check` after a tentative fix to confirm the live count dropped. The check is fast; iterate freely.
...
- **Body.** Include the before/after totals from `cargo make standards-check`, the list of files touched, and — for any target left undone — a one-paragraph rationale that gets mirrored back into [AGENTS.md §"Quarterly migration cadence"](../../AGENTS.md#quarterly-migration-cadence) so the next sweep inherits the context.
