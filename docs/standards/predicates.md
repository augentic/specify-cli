...
| `mod-rs-forbidden` | Any source file named `mod.rs` under `src/` or `crates/`. The 2018-edition `<module>/mod.rs` layout is retired in favour of `<module>.rs` + `<module>/<concern>.rs`; the walker already exempts `tests/`, where `tests/<helper>/mod.rs` is the sanctioned cargo idiom (see [coding-standards.md §"Module layout"](./coding-standards.md#module-layout)). | [`xtask/src/standards.rs`](../../xtask/src/standards.rs) |
