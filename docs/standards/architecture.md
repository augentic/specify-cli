...
The framework authoring checks behind `specify lint framework` are no longer a standalone crate: the dissolved `specify-authoring` now lives as the `specify_standards::framework` module (see [DECISIONS.md §"Crate layout"](../../DECISIONS.md#crate-layout)).
