...
| [`topology-lock.schema.json`](topology-lock.schema.json) | Validates `.specify/topology.lock`, the committed projection of each member project's `project.yaml` topology facets regenerated by `specify workspace sync` (RFC-36). Hub plan-time topology reads this cache rather than `registry.yaml`. |
