...
| [`slice/reconciliation.schema.json`](slice/reconciliation.schema.json) | Validates a slice's `reconciliation.yaml` reconciliation index as the audit surface listing every `REQ-*` id with its contributing `(source, claim-id)` pairs and authority outcome. |
