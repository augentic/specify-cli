...
---
id: discovery
description: Read inputs and emit a neutral adapter inventory.
...
|---|---|---|---|
| `id` | yes | non-empty string | Referenced by `needs` in sibling briefs and by `pipeline.*` entries in the owning `adapter.yaml`. |
...
- [`../adapter.schema.json`](../adapter.schema.json) - schema for the adapter manifest that declares which briefs make up a adapter's pipeline; `pipeline.*` entries reference brief `id` values.
