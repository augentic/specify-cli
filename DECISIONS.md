...
## Idiomatic Rust cleanup (2026-05)

Summary of the R1–R19 "Idiomatic Rust Cleanup" sweep on the `code-review` branch. Each paragraph below is the standard the cleanup locked in, not the chunk history; AGENTS.md has the operational form.

**Handler shape: `Result<()>` + `Ctx` + `report_error`.** `pub type Result<T, E = Error>` lives on `specify-error`; command handlers default to `Result<()>` and rely on the dispatcher in `src/commands.rs` to map `Ok(())` to `CliResult::Success`. Handlers that surface a non-success exit via a typed `*ErrBody` may keep `Result<CliResult, Error>` — those are listed in `scripts/standards-allowlist.toml` under `result-cliresult-default` and shrink as they migrate. The handler context is `crate::context::Ctx` (formerly `CommandContext`; the module path already supplies the noun). Output goes through one entry point: `emit(Stream::Stdout, format, &Body)` for success bodies and `report_error(format, &err)` for failures. `emit_err` / `emit_response` / `emit_error` / `emit_json_error` have all been collapsed into this surface. `CliResult::from(&Error)` is the single source of truth for the four-slot exit table; `CliResult::Code(u8)` is reserved for `specify tool run` WASI passthrough. `ValidationErrBody` is a top-level `Render` impl peer to `ErrorBody`, dispatched directly by `report_error`. (R1, R3, R4, R5, R9)

**DTO discipline: `From<&Domain>`, typed enums, `PathBuf` + `serialize_path`.** Response DTOs (`*Body`, `*Row`) are top-level structs. They are constructed through `impl From<&Domain> for Body` so the conversion is discoverable at the trait surface; named `from_<word>` constructors are reserved for multi-arg or fallible builders (each survivor carries a one-line justification, e.g. `RegistryProposalRow::from_kind` returns `Option<Self>`). Stringly-typed `pub status: String` / `pub kind: &'static str` fields have been replaced with the underlying domain enum (`#[derive(Serialize)]` + `#[serde(rename_all = "kebab-case")]`); the wire shape is unchanged. `*Body` fields that hold filesystem paths are `path: PathBuf` and serialise through `#[serde(serialize_with = "crate::output::serialize_path")]`; no DTO stores `String` paths. (R6, R7, R8)

**Path & I/O: `Layout<'a>` newtype, time injection, hardened `ureq`.** The `.specify/` layout is a `Layout<'a>` newtype in `specify-config`. Path helpers are inherent methods (`Layout::plan_path`, `Layout::change_brief_path`, etc.); call sites write `dir.layout().plan_path()` via the `LayoutExt` trait on `&Path`. New `.specify/` paths land on `Layout`, not as free functions. Functions that record a timestamp into a serialised artifact take `now: chrono::DateTime<Utc>` from the dispatcher boundary; library crates never call `Utc::now()` directly. The WASI tool fetch in `crates/tool/src/resolver.rs` runs every HTTP request with explicit per-call timeouts, a `MAX_RESPONSE_BYTES` cap (64 MiB) checked on both `Content-Length` and the streamed body, and streams to a tempfile before persisting into the cache. (R10, R14)

**Type tightening: `SourceArg`, `YamlError` wrappers.** `--source key=value` arguments are parsed via `impl FromStr for SourceArg` so call sites read named fields instead of tuple positions. `serde_saphyr` deser and ser errors are wrapped behind `specify_error::YamlError` / `specify_error::YamlSerError` so the upstream crate name does not leak through every `specify-*` public surface; `?` on raw saphyr results still works because `Error` carries `From<serde_saphyr::Error>` and `From<serde_saphyr::ser::Error>` impls that go through the wrappers. (R11)

**Lint posture: `missing_debug_implementations` + `unreachable_pub` + `#[expect]`; xtask predicates lock R1/R6/R15.** `[workspace.lints.rust]` adds `missing_debug_implementations`, `unreachable_pub`, `single_use_lifetimes`, and `redundant_lifetimes` at `warn` (under `-Dwarnings` in CI). Site-local lint suppressions are `#[expect(<lint>, reason = "…")]` rather than `#[allow]` so a dead suppression surfaces as a build failure; module-level `#![allow(...)]` waivers remain the right tool when the lint legitimately applies to every item below. Pre-1.0 `#[deprecated]` shims have been deleted — there are no production callers to preserve. Four xtask predicates lock the R1/R6/R15/clap-help-shape standards: `result-cliresult-default` (forbids new `Result<CliResult>` handlers), `from-not-builder` (forbids new `*Body::from_<word>` inherent constructors), `verbose-doc-paragraphs` (caps doc paragraphs on `pub` items at ~8 lines), and `cli-help-shape` (caps clap-derive `///` doc lines at 80 chars). (R15, R16, R16b, R17)

**Bin → lib shim and xtask tooling.** `src/main.rs` is a thin `ExitCode` shim around `specify::run`; the dispatcher and command modules live in `src/lib.rs`. Hosting them in a library lets workspace tooling consume `specify::command()` (the `clap::Command` tree) without spawning the binary. The first consumer is `xtask gen-man`, which emits roff man pages via `clap_mangen`; future tooling (`gen-completions`) goes through `xtask`, not a parallel binary or facade. The `[[bin]]` target shape is unchanged — `cargo install specify` produces the same installable binary. (R19)

**WASI carve-outs.** `crates/contract` and `crates/vectis` are deliberate carve-outs from the workspace's Render/emit/`specify-error` discipline; they ship as standalone WASI components and must stay dependency-minimal. Carve-out comments in `crates/contract/src/main.rs` and `crates/vectis/src/lib.rs` are authoritative. `JSON_ENVELOPE_VERSION` carries a bump-history block in `src/output.rs` so future bumps maintain the log. (R12)

**YAGNI sweep on `CodexRuleFrontmatter`.** Optional fields on `CodexRuleFrontmatter` have been audited against actual consumers (Rust + parent-repo codex rule files); fields with no consumer are deleted, surviving fields carry a doc comment naming the canonical consumer site. Schema (`schemas/codex-rule.schema.json`) updated to match. (R13)
