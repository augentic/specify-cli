...
- **D4 verify-on-read is wired (H2).** `install_tofu` records a deterministic tree content digest of the unpacked store entry into a sibling `<store>/<name>@<version>.meta`; resolve recomputes the digest and verifies it for `AdapterLocation::Store` entries only — raising `adapter-digest-mismatch` — leaving `Local` / `Cached` exempt. The registry layer digest stays the publish-time anchor (verified on the publish pull-back); under single-layer OCI, read-time verification uses the cheaper tree content digest rather than re-tarring the entry.
