...
No framework `CORE-*` rule runs as an in-process check — see [§"Framework lint engine: generic dispatcher (Road A / Road B)"](#framework-lint-engine-generic-dispatcher-road-a--road-b). The former `specify_standards::framework` `Check` substrate (trait, `Context`, `builder.rs`, helpers) is deleted: its only live content was the repo-local Rust-quality predicates, which now live dev-only beside their single consumer at `tests/rust_quality/checks.rs` in the root crate, and the brief path-classification, which `crates/standards/src/lint/index/brief.rs` owns.
...
- **Road B — referenced tool.** The rule carries `kind: tool, value: <tool>` plus a sentinel `path-pattern`. `lint/eval/tool.rs` resolves the tool **by name** from a declared inventory, runs it once per lint, and folds its `DiagnosticReport` (findings stamped with the tool's own `rule_id` / `severity`; the engine restamps only `id` / `fingerprint`). The evaluator sees only the `ToolRunner` trait — for framework lint the inventory is the in-process checker table in the root binary (see the B-2 exit below); for project lint it is the declared WASI tool inventory.
...
**The `kind: authoring-predicate` bridge is gone.** This supersedes the former "imperative predicates run behind a bridge" posture — the `HintKind::AuthoringPredicate` variant, its evaluator, the imperative rule predicates, and the duplicated owner maps are all deleted. CORE-034 (a git-only advisory that emitted no finding) was removed rather than ported; its sibling CORE-031 filesystem header validation lives in the `scenarios` tool.

**Six framework checkers run in-process (B-2 exit, 2026-06-12).** The Road B framework checkers (`scenarios`, `skill-body`, `links-registry`, `marketplace`, `prose`, `rules`) are native Rust modules under [`src/runtime/commands/lint/framework_tools/`](./src/runtime/commands/lint/framework_tools.rs) in the root binary, dispatched by the same name-resolving `ToolRunner` the engine always saw (`FrameworkToolRunner`). The former "B-2 interim posture" — checker crates in `wasi-tools/<name>/` with checked-in `dist/<name>-<ver>.wasm` blobs, `.sha256` sidecars, the `dist_digests_pinned` drift test, the shared `framework-wire` envelope crate, and the `cargo make framework-wasm` pipeline — is fully retired: the WASM hop bought sandboxing for first-party code reviewed in this repo while costing a build pipeline, binary blobs in git, and a digest-pinning debate. The wire contract is unchanged: each checker still receives `(candidate-path, config-json)` argv, reads policy only from the rule's forwarded `config:`, and returns a `DiagnosticReport` envelope the `kind: tool` evaluator folds (restamping `id` / `fingerprint`). The `agent-teams` checker was retired with CORE-012 rather than ported. Ported fixes: `requested_rule` matches the candidate filename exactly instead of substring-scanning all argv; the prose checker parses `description.maxLength` from the canonical embedded skill schema instead of substring-matching `"512"`; a missing project dir surfaces findings instead of silently passing. `specify-standards` stays wasmtime-free and policy-free — it sees only the `ToolRunner` trait. WASM remains for the adapter validators (`contract`, `vectis`), which ship to consumer projects and keep the sandbox + digest sidecar trust anchor.

Test coverage rests on the per-kind evaluator suite (`crates/standards/tests/lint_hint/*.rs`), the `no_embedded_policy` guard, the schema byte-match gate (`crates/schema/tests/schemas.rs`), and each checker module's unit tests; the cutover-only parity suites were deleted.
...
| `lint-completed`                                        | `specify lint project` after each scan (the `specify lint framework` development surface does **not** journal — it sets `journal: false` in [`src/output.rs`](./src/output.rs)); payload carries `scope`, `duration_ms`, per-status `counts.{open, ignored, false_positive}`, `baseline_present` (hard-coded `false` until RFC-33b lands), and the resolved `exit_code`. Wire field names are snake_case to match the journal payload verbatim. |
