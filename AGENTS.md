...
- `crates/workflow/src/journal.rs` — newline-delimited JSON journal event log at `<project_dir>/.specify/journal.jsonl`; closed `Event` / `EventKind` taxonomy with kebab-case wire ids and `snake_case` Rust variants joined by `#[serde(rename = "…")]` (including the single `PlanReconcileCompleted` variant covering a successful `plan propose --from` write, plus the bootstrap events `cli.upgraded` / `plugins.refreshed` / `migration.applied` / `migration.skipped`).
