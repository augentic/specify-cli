...
- `crates/workflow/src/change/plan/core/propose.rs` — plan-time lead-reconciliation kernel. Envelope DTOs (closed `kind: request | response`), the pure `build_request` / `build_catalog` / `resolve_topology` assembly, and the `Plan::propose_from` projection kernel behind `specify plan propose --dry-run | --from`. See [`DECISIONS.md` §"Lead reconciliation (D2)"](./DECISIONS.md#lead-reconciliation-d2).
- `crates/workflow/src/slice/build/` — target build envelope kernel. `wire.rs` holds the closed-shape `BuildRequest` / `BuildReport` DTOs (round-tripping `schemas/target/build-{request,report}.schema.json`) plus the `enforce_report_no_blocking_on_success` gate; `assemble.rs` assembles a request from the bound target adapter's declared `inputs[]` against the slice tree (raising `target-build-input-missing`). The `specify slice build <slice> [--phase prepare|finalize]` handler ([`src/runtime/commands/slice/build.rs`](./src/runtime/commands/slice/build.rs)) owns request assembly, report validation, the `target-build-*` aborts, the `slice.build.*` events, and the `built` transition gate.
...
- `crates/standards/src/framework/` — the dissolved `specify-authoring` crate: the imperative `Check` predicates behind `specify lint framework` (`check/`, `context.rs`, `helpers.rs`, `schema.rs`, `error.rs`). `builder.rs` holds the `CORE_ID_TABLE`, severity table, and `framework_finding()` / `loc()` builders; every predicate emits a canonical `Diagnostic` directly, and `framework::check::run` finalizes (rebase → fingerprint → `FIND-NNNN` ids). These imperative predicates are the **intended steady state**: the declarative burn-down is bounded by the one-discriminator-per-hint-kind engine constraint (each fact-consuming hint kind is hardcoded to a single `source` discriminator, all spent on `CORE-001..009`), so only a handful more ids are cleanly migratable author-side and the rest need new hint discriminators + indexer facts (a future RFC) — do not retire predicates or remove `AuthoringProducer` by weakening checks. See [DECISIONS.md §"Crate layout"](./DECISIONS.md) (framework-authoring-checks paragraph).
- `crates/standards/src/lint/` — `specify lint` and `specify lint framework` surface: `WorkspaceModel` DTOs (`model.rs`), the dual-profile indexer (`index/`), the deterministic hint interpreter for the closed Phase 2 kinds (`eval/{path_pattern,regex,schema,tool}.rs`), and the shared lint runner. The renderers it returns are the neutral formatters from `specify-diagnostics`. The framework-profile extractors (`index/{skill,adapter,marketplace,agent_teams,brief}.rs`) sit beside the consumer pass and run when `lint::index::build(project_dir, ScanProfile::Framework, &[], &[])` is invoked; the §F1 walk driver lives in `index/framework.rs` and follows symlinks (recording both endpoints) while the consumer profile records-without-traverse. `specify lint framework` is the only caller of the framework profile today.
...
```text
src/runtime/          specify dispatch (the single CLI; lint run + lint framework)
crates/workflow/        workflow domain logic
crates/standards/src/framework/  imperative framework check predicates (behind specify lint framework)
```
