...
- `crates/domain/src/codex.rs` + `crates/domain/src/codex/` — `CodexRule`, `CodexResolver`, `ResolvedCodex`, `CodexProvenance`. Project-aware codex rule catalog routed through `TargetAdapter::resolve(name, …)` for the foundational `default` adapter and the project's declared adapter.
