...
- `crates/standards/src/framework/` — the dissolved `specify-authoring` crate: framework `Check` predicates (`check/`, `context.rs`, `helpers.rs`, `schema.rs`, `error.rs`). **RFC-31 steady state:** `check::run` is the CORE-009 namespace bridge only; `CORE-010..052` run via declarative `kind: authoring-predicate` on `CORE-*` rule files in augentic/specify (`lint/eval/authoring_predicate.rs`). `builder.rs` holds the `CORE_ID_TABLE` (CORE-009 only), severity table, and `framework_finding()` builders. Posture: [DIAGNOSTICS.md §A16](./DIAGNOSTICS.md), [DECISIONS.md §"Crate layout"](./DECISIONS.md). Parity contract (framework repo): [docs/contributing/checks.md](https://github.com/augentic/specify/blob/main/docs/contributing/checks.md#parity-contract-for-predicate-retirement). Engine spikes: [rfc-31-phase1-spike.md](./docs/standards/rfc-31-phase1-spike.md), [rfc-31-phase2-spike.md](./docs/standards/rfc-31-phase2-spike.md).
- `crates/standards/src/lint/` — `specify lint` and `specify lint framework` surface: `WorkspaceModel` DTOs (`model.rs`), the dual-profile indexer (`index/`), the deterministic hint interpreter for the closed Phase 2 kinds (`eval/{path_pattern,regex,schema,tool}.rs`), and the shared lint runner. The renderers it returns are the neutral formatters from `specify-diagnostics`. The framework-profile extractors (`index/{skill,adapter,marketplace,agent_teams,brief}.rs`) sit beside the product pass and run when `lint::index::build(project_dir, ScanProfile::Framework, &[], &[])` is invoked; the §F1 walk driver lives in `index/framework.rs` and follows symlinks (recording both endpoints) while the product profile records-without-traverse. `specify lint framework` is the only caller of the framework profile today.
