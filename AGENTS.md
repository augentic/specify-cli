...
- `crates/standards/src/framework/` — the dissolved `specify-authoring` crate: framework `Check` predicates (`check/`, `context.rs`, `helpers.rs`, `schema.rs`, `error.rs`). RFC-31 Phase 4: `check::run` is the CORE-009 namespace bridge only; migratable ids run via declarative `kind: authoring-predicate` on `CORE-*` rule files in augentic/specify (`lint/eval/authoring_predicate.rs`). `builder.rs` holds the `CORE_ID_TABLE` (CORE-009 only), severity table, and `framework_finding()` builders. Phase 1 spike: [docs/standards/rfc-31-phase1-spike.md](./docs/standards/rfc-31-phase1-spike.md). See [DECISIONS.md §"Crate layout"](./DECISIONS.md).
