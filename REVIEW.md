...
- **S4** — predicted ≈ −2 LOC cross-crate, actual −6 LOC across the three touched files (`src/commands/tool/dto.rs` −11, `crates/tool/src/manifest.rs` +4, `crates/tool/src/lib.rs` 0 net = +13 / −7). The reviewer's manifest.rs baseline was stale (322 cited vs 351 actual; no S4-relevant churn between, just a drift in the baseline number) so the delta-to-final reads cleanly but the absolute totals do not match the review. Slight LOC overshoot vs the inline-attribute spec because I used four separate `#[strum_discriminants(...)]` attribute lines instead of the review's single combined block — easier to read, one line shorter (5 vs 6 attribute lines). Both done-when assertions flipped cleanly: `rg -c 'enum ToolScopeKind' src/ crates/` → `0` and `rg 'ToolScopeKind' src/ crates/ -t rust` shows only uses + the `name(ToolScopeKind)` declaration inside the derive attribute (no `enum` definition). `scope_labels` retained its explicit `match` per the action note — `ToolScopeKind::from(scope)` is available via the derive but the function still has to extract `scope_detail`. AGENTS.md item 4 doc-sweep (`rg ToolScopeKind AGENTS.md DECISIONS.md docs/`) was a no-op. `cargo make lint` clean; `cargo make test` 825/825 pass; no behavioural regressions (`Serialize`/`Deserialize`/`Display`/`Hash` shape preserved end-to-end through the JSON DTO callers).
- **T1** — predicted −3 LOC, actual −4 LOC in `src/commands/slice/merge.rs` (function 8→6 plus doc-comment 8→6); the review costed the function body only and missed that the "two-part heuristic" doc-comment also referenced the removed `has_project_yaml` line and had to be rewritten. Done-when `rg 'has_project_yaml' src/commands/slice/merge.rs` → `0` flipped cleanly. `cargo make ci` clean; the three existing tests (`workspace_clone_path`, `rejects_normal_project_root`, `rejects_bare_specify_dir`, `deeply_nested_workspace_clone`) still pass — they were already constructed without `plan.yaml`, so the predicate's narrower form (`!plan_path().exists()` only) returns the same value at every input. No regressions.
- **T3** — predicted −6 LOC, actual −4 LOC in `src/commands/slice/merge.rs` (helper deleted: −7 lines for `fn merge_pathspecs` body and signature; inline call site: +3 lines for the `let pathspecs: Vec<&'static str> = ...` chain spread across 5 lines vs the prior 1-line `merge_pathspecs(project_dir)` call). The review predicted a single-line inline form; in practice keeping the `Vec<&'static str>` annotation and chaining four method calls reads better as five lines. Done-when `rg 'fn merge_pathspecs' src/commands/slice/merge.rs` → `0` flipped cleanly. `cargo make ci` clean; `auto_commit` still drives `git add`/`diff --cached`/`commit` through the same filtered pathspecs. No regressions. Combined T1+T3 file delta: +11 / −19 = −8 LOC across one file.
