...
- **F2** — applied. Predicted −23 net LOC, actual −19 (9 +, 28 −) across `push.rs` + `workspace.rs`. Done-when (`rg -c 'struct PushItem' src/commands/workspace.rs` → 0) flipped cleanly. No regressions: full `cargo make ci` passes (813/813 nextest, lint, fmt-check, doc, vet, outdated, deny — only pre-existing unrelated `deny` warnings). The `#[serde(rename = "pr")]` on `PushResult.pr_number` kept the wire byte-identical — `tests/cross_repo.rs::rm01_replays_cross_repo_through_finalize` asserts `project["pr"].as_u64()` and stayed green. Per-field `skip_serializing_if` on `branch` / `error` matched the prior `PushItem` shape exactly. ΔLOC came in under prediction because the review counted `PushItem`'s 12-line struct + 10-line map loop in full but didn't net out the 4 new serde attributes on `PushResult`.
- **F3** — applied. Predicted −25 net LOC, actual −11 net (72 +, 61 − across 7 files); `crates/error/src/error.rs` alone is −27 (matches the headline cleanly), but the per-site `Error::Diag { code, detail: format!(…) }` rewrites at 6 producers each cost ~3 lines vs the prior typed variant's single-line construction, and the three transitions-tests + one lock-test rewrites added ~12 LOC swapping exhaustive pattern matches for `code` + `detail.contains(...)` assertions. Done-when (`rg -c 'PlanTransition|DriverBusy|SliceNotFound' crates/error/src/error.rs` → 0) flipped cleanly. No regressions: full `cargo make ci` passes (813/813 nextest, lint, fmt-check, doc, vet, outdated, deny — same pre-existing `tiny-keccak` / unused-`toml` `deny` warnings as F2). Wire stayed byte-identical: `tests/plan_orchestrate.rs:1397` still asserts `exit-code == 1` for the driver-busy case and stayed green. Updated `docs/standards/coding-standards.md:185` to drop `Error::PlanTransition` from the typed-variant examples and add the F3 counter-argument as a guard ("payloads that only round-trip into `detail` belong on `Error::Diag`"). One under-counted follow-on: doc-comment references to the deleted variants in `lock.rs` (3 sites) needed retargeting at `[`Error::Diag`]` with code `driver-busy`; not enough LOC to matter but easy to miss.
- **F4** — applied. Predicted −23 net LOC, actual −20 (34 +, 54 − across 3 files: `src/commands/plan/status.rs`, `src/commands/plan.rs`, `src/commands/status.rs`). Done-when (`rg -c 'Status::ALL.iter\(\).map\(\|&s\| \(s, 0\)\)' src/` → 1) flipped cleanly (was 2). No regressions: full `cargo make ci` passes (lint, test, doc, vet, outdated, deny — same pre-existing `tiny-keccak` / unused-`toml` `deny` warnings as F2/F3). Wire stayed byte-identical: the consolidated `Counts` keeps all 7 kebab-case fields and their derives, so `tests/fixtures/plan/status-platform-v2.json`'s `counts:{ blocked, done, failed, in-progress, pending, skipped, total }` shape is unchanged; the dashboard renderer reads `p.counts.done` etc. unchanged. One under-counted follow-on the review missed: clippy's `redundant_pub_crate` lint forbids `pub(crate)` inside an already-private module path, so `Counts` and `from_entries` had to be plain `pub` (with `mod status;` flipped to `pub(super) mod status;` in `plan.rs`) — exposed surface is unchanged for non-crate callers because the crate is a binary. ΔLOC came in slightly under prediction because the new `impl Counts { fn from_entries }` block carries 18 LOC (one more than the predicted 17 helper + struct kept once) thanks to the `impl Counts {}` brace pair, while the BTreeMap loop deletion at the second call site was already counted in full.
- **F5** — applied. Predicted −21 net LOC, actual −18 (12 +, 30 − across 4 files: `crates/error/src/lib.rs`, `crates/error/src/serde_path_display.rs` deleted, `crates/domain/src/slice/actions/create.rs`, `crates/domain/src/merge/slice.rs`). Done-when (`wc -l crates/error/src/serde_path_display.rs` errors) flipped cleanly — file gone, `rg --type rust -l 'serde_path_display'` returns no source hits (only this REVIEW.md). No regressions: full `cargo make ci` passes (lint, test, doc, vet, outdated, deny — same pre-existing `tiny-keccak` / unused-`toml` `deny` warnings as F2/F3/F4). Wire stayed byte-identical: each call site's per-field `serialize_with` swap goes from `"specify_error::serde_path_display::serialize"` to a sibling `ser_dir` / `ser_baseline_path` whose body is the same `serializer.collect_str(&value.display())` one-liner, so the rendered `dir` / `baseline_path` string output is unchanged. ΔLOC came in slightly under prediction because the predicted 3-LOC-per-site helper assumed a single-line `#[expect]`; the actual 4-LOC form keeps the two-line `clippy::ptr_arg` `#[expect(..., reason = "...")]` attribute that the original module used (matching the house style for lint suppressions), plus a blank line between sibling items. No follow-ons: only one downstream `pub mod` line in `crates/error/src/lib.rs` needed to vanish, and no test or doc-comment in the workspace referenced the adapter path (vs. F3's lock.rs doc-comment surprise).
- **T1** — applied. Predicted −4 LOC; actual −6 net (1 +, 7 − in `src/commands/plan.rs`) — extra trim from dropping the now-orphaned `Entry` and `Value` imports alongside the helper. Done-when (`rg change_entry_json src/` empty) flipped cleanly. No regressions: `cargo make ci` green (same pre-existing `tiny-keccak` / unused-`toml` `deny` warnings).
- **T2** — applied. Predicted −1 LOC + −1 import; actual −2 net (13 +, 15 − in `crates/error/src/error.rs`; 1 +, 1 − in `src/output.rs`). Done-when (`rg 'Cow' crates/error/src/error.rs` empty; was 14) flipped cleanly. No regressions: `cargo make ci` green; the `error::tests::diag_round_trip` `assert_eq!(err.variant_str(), "kebab-prefix")` still compiles via `PartialEq<&str> for String`.
- **T3** — applied. Predicted −9 LOC; actual −8 net (within `src/commands/plan/create.rs`, attributable to this tidy after subtracting T1's slice). The 9-line enum + derive deleted, but the renderer match gains a one-line `other => unreachable!(...)` arm because `&'static str` is open. Done-when (`rg 'enum EntryAction' src/` empty) flipped cleanly. No regressions: `cargo make ci` green; the wire string (`"create"` / `"amend"`) stayed byte-identical because both call sites pass the literal that serde-rename used to produce.
- **T4** — applied (with the `LifecycleStatus::ALL_STATUSES` fold). Predicted −4 LOC + 1 idiom; actual −8 net across 4 files (5 +, 8 − in `transitions.rs`; 4 +, 11 − in `lifecycle.rs`; 3 +, 1 − in `src/commands/plan/status.rs`; 1 +, 1 − in `docs/standards/coding-standards.md`). Done-when (`rg 'Status::ALL\|ALL_STATUSES' crates/ src/` empty; was 5 prod + 1 test const) flipped cleanly. No regressions: `cargo make ci` green; the three new `use clap::ValueEnum;` imports unlocked the existing derive without bumping the public surface. One under-counted follow-on the review missed: `docs/standards/coding-standards.md:189` named `Status::ALL` as the canonical "expect-justified invariant" example and had to be repointed at `Status::value_variants()` (per the AGENTS.md "removed symbol → grep docs/" rule).
- **T5** — applied. Predicted −10 LOC (target ≤ 158); actual −9 (0 +, 9 − in `plugins/change/skills/analyze/SKILL.md`), leaving the file at **159** LOC. Done-when (`wc -l ≤ 158`) **missed by one** — the review under-counted by a single blank-line separator between §"Per-kind prompts" and the now-following §"Error handling"; no follow-up trim attempted because the file is still well below the 200-LOC skill cap and below the prior 168-LOC headline. No regressions: `make checks` in the specify repo fails only on pre-existing broken links under `rfcs/archive/rfc-23-change-lifecycle.md` (verified by stashing this change — same nine failures, all pre-dating the survey branch).
