...
- **S2**: predicted −10 LOC, actual **−15 LOC** (5 insertions, 20 deletions across 2 files: `crates/tool/src/load.rs` −13, `src/commands/tool/dto.rs` −2); done-when flipped clean (`rg "Collision" crates/tool/ src/` empty; `cargo nextest run -p specify-tool load::tests` 4/4 green; full sweep 292/292 workspace tests green); no regressions. Calibration: prediction undercounted by ~5 LOC because the test-fixture `vec![Collision { name: ... }]` literal (4 lines → 1 line) wasn't separately accounted for, and the struct's two doc-comment lines (`///` on the type and `///` on `name`) were folded under "−1 public type" rather than tallied. The `dto.rs` call-site touch was a 4-line block → 0-line (use removal) + 1-line param swap, exactly matching the action list. Forward note: when a single-field newtype has both a struct-level and a field-level doc comment, count both in the LOC envelope.
- **S3**: predicted −6 LOC, actual **−7 LOC** (5 insertions, 12 deletions across 2 files: `crates/domain/src/init/cache.rs` −5, `crates/domain/src/init/regular.rs` −0); done-when flipped clean (`rg "CachedCapability" crates/domain/src/init/` empty; `cargo nextest run -p specify-domain` 469/469 green); no regressions. Calibration: near-perfect — the struct had no field-level doc-comment (unlike S2), so the LOC envelope held within 1 line. The `Ok(CachedCapability { capability_value: ... })` → `Ok(source.capability_value)` collapse saved 2 lines beyond the bare struct deletion, exactly offsetting the regular.rs rename's net-zero. Forward note: when a single-field newtype has zero doc-comments and the construction is multi-line braces, the bare struct delete (4 lines: derive + struct head + field + close) plus return-site collapse (3 lines → 1 line) lands at ≈−6 LOC reliably.
- **S4**: predicted −20 LOC, actual **−43 LOC** (75 insertions, 118 deletions in `crates/tool/src/validate.rs`, 511 → 468); done-when flipped clean (`rg "^fn (pass|fail)\(" crates/tool/src/validate.rs` empty; `#[expect(clippy::too_many_lines)]` lifted and clippy still green; `cargo nextest run -p specify-tool validate::tests` 10/10; full sweep 837/837 workspace tests green); no regressions (`cargo make ci` only failed on the offline `vet` step due to DNS unavailability). Calibration: prediction undercounted by ~23 LOC because (a) collapsing four `(bool, String)` tuple bindings to `Option<String>` via `.then()` / `.err().map()` shaved 2-3 lines per binding (not the budgeted 1), (b) the eleven `check(rule, "...", valid, || detail)` call sites each lost the `|| detail` line plus rustfmt re-flowed several to two-line form, and (c) the three `validate_*` if/else tails compressed to single `check(...)` invocations more aggressively than the `−1 branch` budget assumed. The action list's "remove `#[expect]` to verify shrink" step required a one-line tightening pass (collapsing the `package` `match` to `if let` and joining `name_detail` to one line) to clear 100/100 — i.e. the body landed at 103 lines after the headline refactor and needed two trims to drop under the lint. Forward note: when collapsing `(bool, String)` accumulators to `Option<String>` plus inlining helper-fn bodies, budget −3 lines per accumulator (not −1) and assume the `clippy::too_many_lines` lift will need a second tightening pass at the boundary.
- **T1**: predicted −22 LOC, actual **≈−17 LOC** (helper +17 in `crates/tool/src/lib.rs`; 11 call sites collapsed across `resolver.rs`, `resolver/digest.rs`, `resolver/http.rs`, `resolver/local.rs`, `cache/tests.rs`); done-when flipped clean (`rg "EnvGuard::unset\(\"HOME\"\)" crates/tool/src/` returns exactly the helper match); 837/837 workspace tests green, no regressions. Calibration: prediction undercounted helper docs (5 lines of `///`) and the `pub fn` signature itself (4-line wrap for the `(MutexGuard, [EnvGuard; 3])` return type). Also: REVIEW.md's drop-order claim was inverted — Rust drops tuple fields in declaration order (per the reference), so the helper had to return `([EnvGuard; 3], MutexGuard<'static, ()>)` (not the suggested `(MutexGuard, [EnvGuard; 3])`) so the env-restoring guards drop before the lock releases. Forward note: when adding a returned-tuple drop-ordered guard helper, write the destructive resource last in the tuple and verify against the Rust reference rather than trusting the proposal's drop-order intuition.
- **T2**: predicted −4 LOC, actual **−2 LOC** (`crates/tool/src/package.rs` +5/−7, plus 1-line `Some(...)` swap at `resolver.rs:51`); done-when flipped clean (`rg "Option<PathBuf>" crates/tool/src/package.rs` empty); no regressions, full workspace clippy + 837/837 tests green. Calibration: prediction overcounted by 2 because the docstring on `WasmPkgClient::new` shrank by only 2 lines (the "Pass `None` when..." paragraph) — the rest was a 1-for-1 type swap with no LOC churn.
- **T3**: predicted −2 LOC, actual **≈−1 LOC** (`crates/tool/src/manifest.rs` +5 helper with 1 doc line; `crates/tool/src/cache/meta.rs` −3 deletes the local `valid_sha256`; `crates/tool/src/validate.rs` −2 collapses the 4-line `.filter` to a 1-line predicate; both files take +1 line each from re-flowed `use` blocks); done-when flipped clean (`rg "\(b'a'\.\.=b'f'\)" crates/tool/src/` returns one match — the hoisted helper). Calibration: prediction nailed the headline saving but didn't budget for the rustfmt re-flow of the two `use` blocks adding the new symbol; for hoisted-helper tidies that touch >1 import site, add +1 LOC per import for re-wrap.
- **T4**: predicted −4 LOC, actual **−5 LOC** (`src/commands/init.rs` -5: the 4-line `debug_assert!` plus the trailing blank); done-when flipped clean (`rg "debug_assert.*hub.*capability" src/` empty; `cargo test -p specify --test init` 7/7 — the existing `init_with_no_args_errors` and `init_with_capability_and_hub_errors` cover the same xor envelope the deleted assert was double-checking, and the diagnostic still emits from `domain::init::run`). Calibration: spot on; the predicted-budget vs actual delta of −1 was the trailing blank line.
- **T5**: predicted −7 LOC, actual **+6 LOC** (`crates/tool/src/host.rs` +18/−12 — removed the 10-line `path_to_env` helper but each of the two call sites grew from 1 line to 8-line builder-arg blocks because rustfmt insists on multi-line form for `builder.env(KEY, val.to_str().ok_or_else(|| ToolError::invalid_permission(...))?)`); done-when flipped clean (`rg "fn path_to_env|path_to_env\(" crates/tool/src/host.rs` empty); 837/837 tests green, clippy clean, no regressions. Calibration: prediction inverted the actual direction. Lesson: when the helper body returns through `?` from inside a multi-arg method call, the call sites cannot collapse onto one line after inlining (rustfmt re-wraps the closure across 4-5 lines plus the `builder.env(\n KEY,\n value,\n)` envelope adds 3 more), so two call sites will inflate well past the helper's 8-10 lines. Forward note: only take "inline single-call-site helper" tidies when the surrounding call site is itself single-line — never when it's mid-method-chain or has a closure that returns via `?`. The helper saves more than it costs in those shapes.
- **T6**: predicted −15 LOC, actual **≈−30 LOC** (`crates/tool/src/cache/meta.rs` +7/−43 — most of the saving from deleting `Sidecar::new`'s 33-line impl block including the 6-line doc-comment + 4-line `#[expect(clippy::too_many_arguments)]` + 8-line constructor signature with `impl Into<String>` bounds; +1 in `cache.rs` re-exporting `SIDECAR_SCHEMA_VERSION`; +1 net in `resolver.rs` for the inline struct literal; +1 net in `cache/tests.rs::fixed_sidecar`); done-when flipped clean (`rg "fn new\(" crates/tool/src/cache/meta.rs` empty; full 837/837 workspace tests green including `cache_status_distinguishes_*`, `package_source_uses_injected_client_and_records_metadata`, and the round-trip `sidecar_round_trips_and_schema_rejects_invalid_shape`). Calibration: prediction undercounted by ~15 because (a) the `#[expect]` lint + its 5-line `reason = "..."` string collapsed too, (b) the impl-block keyword-and-brace scaffolding around a single function adds 3-4 lines that aren't visible in the function body, and (c) REVIEW.md described `Sidecar::new` as "constructed at exactly one site" but `cache/tests.rs::fixed_sidecar` was a second caller — inlining there saved another ~3 lines instead of regressing. Forward note: when a constructor with `impl Into<String>`-bounded params and a `#[expect(too_many_arguments)]` suppression collapses to a struct literal, count the impl-block scaffolding (doc-comment, expect-reason, signature wrap) at +12-15 lines beyond the function body — they often dwarf the body itself. Also: always grep for `Type::new(` rather than trusting the action's call-site count; "single call site" is frequently "single non-test call site".
